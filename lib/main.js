...
exports.main = function(options) {
    ...
    dump('first_startup: ' + prefs.prefs['first_startup'] + '\n');
    ...
    require('page-mod').PageMod({
        ...
        onAttach: function(worker) {
            tabs_workers.push(worker);

            worker.port.on("is_observe_start", function(){
                if(prefs.prefs['click_to_dial_enabled'] && is_user_auth){
    ...
    mainpanel.port.on('setuserdestination', function(value) {
        ...
        dump('setuserdestination value: ' + value + '\n');
        if(value == null) {
...
function mainPanelShow(){
    utils.getMostRecentBrowserWindow()
            .document.getElementById('queuesize').click();
}
...
function startQueueTimer(){
    timer.clearTimeout(queue_timer);
...
function logout() {
    passwords.search({
        ...
        onComplete: function onComplete(credentials) {
            ...
            for(var i in tabs_workers){
                try{
                    tabs_workers[i].port.emit('stop_observe');
                }
                catch(e)
                {
                    tabs_workers.splice(i, 1);
                }
            }
            ...
            onComplete: function(response) {
                ...
                if (response.status == 200) {
                    ...
                    if (userDestinations == null || userDestinations.length == 0) {
                        ...
                        dump(selected_fixed + ' ' + selected_phone + '\n');
                        ...
                        dump(json.fixedDestanations.length + ' ' + json.phoneAccounts.length + '\n');
                        ...
                        dump(json.selectedFixed + ' ' + json.selectedPhone + '\n');
                        ...
                        if(prefs.prefs['click_to_dial_enabled']){
                            for(var i in tabs_workers){
                                try{
                                    tabs_workers[i].port.emit('start_observe');
                                }
                                catch(e)
                                {
                                    tabs_workers.splice(i, 1);
                        ...
                        mainpanel.port.emit('update-user-name', credentials.username);
...
function selectuserdestination(type, id) {
    ...
    var callbackComplete = function(credentials) {
        ...
        dump("  selected_fixed: " + selected_fixed + '\n');
        dump("  selected_phone: " + selected_phone + '\n');
