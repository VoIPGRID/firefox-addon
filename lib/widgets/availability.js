(function() {
    ...
    exports.init = function() {
        ...
        panels.on('availability.select', function(type, id) {
            console.info('availability.select');
            selectUserdestination(type, id);
        });
        panels.on('availability.toggle', function(type, id) {
            console.info('availability.toggle');

            selectUserdestination(type, id);
            panels.emit('availability.refresh');
        });
    };
        ...
        api.asyncRequest(
            ...
            'get',
            {
                ...
                onOk: function(response) {
                    ...
                    panels.emit('availability.reset');
                    ...
                    if(options.length) {
                        panels.emit('availability.fill', options);
                    }
                    ...
                    storage.widgets.availability.icon = icon;
                    if(storage.widgets.queues && !storage.widgets.queues.selected) {
                        main.setToggleButtonProperties({
                            icon: data.url(icon),
                        });
                    }
    ...
    exports.reset = function() {
        panels.emit('availability.reset');
        main.setToggleButtonProperties({
            icon: data.url('assets/img/call-gray.png'),
        });
    };
